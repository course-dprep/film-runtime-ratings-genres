...
# Convert rating_category to numeric for exploratory analyses, since these analyses require numeric DV's
...
linear_regression <- lm((rating_num ~ runtime_min), cleaned_movies)
sink(file.path(analysis_output_dir, "linear_regression_summary.txt"))
summary(linear_regression)
sink()

# exploratory non-linear model: quadratic model
quadratic_model <- lm(rating_num ~ runtime_min + I(runtime_min^2), cleaned_movies)
sink(file.path(analysis_output_dir, "quadratic_model_summary.txt"))
summary(quadratic_model)
sink()

# exploratory non-linear model: logarithmic model
logarithmic_model <- lm(rating_num ~ log(runtime_min), cleaned_movies)
sink(file.path(analysis_output_dir, "logarithmic_model_summary.txt"))
summary(logarithmic_model)
sink()

# ordinal logistic regression seems the best though, because it doesn't assume equal spacing between categories
ordinal_log_regression <- polr(rating_category ~ runtime_min, data = cleaned_movies, Hess = TRUE)
...
ordinal_log_regression2 <- polr(rating_category ~ runtime_min * genre_list, data = cleaned_movies, Hess = TRUE)
